...
async def check_match(*, db_session: DbSession, source_profile: UUID, target_profile: UUID):
    ...
    if is_match:
        match_data = MatchInCreateSchema(profile_1=source_profile, profile_2=target_profile)
        await CreateMatch(repository=MatchRepository(db_session=db_session)).execute(
            in_schema=match_data
        )
        await DeleteMutualLikes(repository=LikeRepository(db_session=db_session)).execute(
